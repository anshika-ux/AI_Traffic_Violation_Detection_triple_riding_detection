...
def process_frame_with_model(frame_path, model_path, args):
    """Process a single frame with its own model instance."""
    ...
    # Save annotated frame only for violations
    if violations > 0:
...
def group_riders_with_vehicles(persons: List[Dict], vehicles: List[Dict],
    ...
    for vehicle in vehicles:
        ...
        # Improved violation detection: check for 3+ riders AND ensure they are properly positioned
        is_violation = False
        if len(riders) >= 3:
            # Additional check: ensure riders are not all clustered in one area (false positive)
            rider_centers = []
            for r in riders:
            ...
            # Calculate spread of riders
            if len(rider_centers) > 1:
                centers = np.array(rider_centers)
                spread = np.max(centers, axis=0) - np.min(centers, axis=0)
                # Require minimum spread to avoid false positives from overlapping detections
                min_spread = v_width * 0.3  # At least 30% of vehicle width spread
...
def print_detection_report(frame_name: str, vehicle_groups: List[Dict], color: bool = True):
    """Print a formatted detection report for a frame with optional colors."""
    sep = "=" * 50
    print("\n" + sep)
    if color:
...
def main(args=None):
    if args is None:
        ...
        p.add_argument("--collection-name", default="detections", help="MongoDB collection name")
        args = p.parse_args()
    ...
    # Create temporary directory for video download and frame extraction
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)

        # Download video from Kaggle or use local file
        if args.video:
        ...
        # Delete the video file after extraction
        video_path.unlink()
        ...
        # For CSV report: frame_number, total_vehicles, total_persons, violation_count
        ...
        # Process frames in parallel using processes (since YOLO model is not thread-safe)
        num_workers = min(multiprocessing.cpu_count(), len(frame_paths))

        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            futures = [executor.submit(process_frame_with_model, frame_path, model_path, args) for frame_path in frame_paths]
            for future in concurrent.futures.as_completed(futures):
                result = future.result()
                if result:
                    report_rows.append(result)

                    # Store result in database if connected
                    if db_manager:
                        frame_number, vehicles, riders, violations = result
                        result_doc = {
